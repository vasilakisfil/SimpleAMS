...
RSpec.describe SimpleAMS::Options, 'fields' do
  ...
  context 'with various injected fields' do
    ...
    it 'holds the union of injected and allowed fields' do
      puts "Expect #{(UserSerializer.attributes & @injected_options[:fields]).uniq}"
  ...
  context 'with custom injected fields' do
    before do
      UserSerializer.attributes(
        :vitae, :dignissimos, :illum, :cum, :voluptates,
        { similique: %i[pariatur vel] },
        :molestias, :qui
      )
      @injected_options = Helpers.random_options(with: {
        serializer: UserSerializer,
        fields: %i[microposts_count created_at]
      })
      @options = SimpleAMS::Options.new(
        User.new,
        injected_options: @injected_options
